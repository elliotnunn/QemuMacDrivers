...
void *backbuf, *frontbuf, *cursorbuf;
int W, H;
...
static OSStatus initialize(DriverInitInfo *info) {
	...
	// Create the cursor resource VIRTIO_GPU_CMD_RESOURCE_CREATE_2D.
	{
		struct virtio_gpu_resource_create_2d *buf = (void *)obuf;
		memset(buf, 0, sizeof(*buf));
		buf->hdr.le32_type = EndianSwap32Bit(VIRTIO_GPU_CMD_RESOURCE_CREATE_2D);
		buf->hdr.le32_flags = EndianSwap32Bit(VIRTIO_GPU_FLAG_FENCE);
		buf->le32_resource_id = EndianSwap32Bit(57); // guest-assigned
		...
		lprintf("Reply to (cursor) VIRTIO_GPU_CMD_RESOURCE_CREATE_2D is %#x\n",
			EndianSwap32Bit(ibuf->le32_type));
	}

	// Attach our cursor backing store
	{
		struct virtio_gpu_resource_attach_backing *buf = (void *)obuf;
		struct virtio_gpu_mem_entry *buf2 = (void *)((char *)buf + sizeof(*buf));
		memset(buf, 0, sizeof(*buf) + sizeof(*buf2));
		...
		buf2->le32_addr = EndianSwap32Bit((uint32_t)cursorbuf + 0x4000);
		buf2->le32_length = EndianSwap32Bit(64*1024*1024);

		VTSend(0, 0);
		while (!VTDone(0)) {}

		lprintf("Reply to (cursor) VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING is %#x\n",
			EndianSwap32Bit(ibuf->le32_type));
	}
...
struct CursorColorTable cursColorTab;
...
static OSStatus SetHardwareCursor(VDSetHardwareCursorRec *rec) {
	...
	for (i=0; i<curs.cursorWidth*curs.cursorHeight; i++) {
		...
		cursorHash ^= *((uint32_t *)cursorbuf + i);
	}
	...
	for (y=64-1; y>=0; y--) {
		uint32_t *oldRow = (uint32_t *)cursorbuf + y*curs.cursorWidth;
		uint32_t *newRow = (uint32_t *)cursorbuf + y*64;
	...
	for (y=0; y<curs.cursorHeight; y++) {
		for (x=0; x<curs.cursorWidth; x++) {
			uint32_t *pixel = (uint32_t *)cursorbuf + y*64 + x;
	...
	// Use VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D to update the host resource from guest memory.
	{
		...
		buf->le32_resource_id = EndianSwap32Bit(57);

		VTSend(0, 0);
		while (!VTDone(0)) {}

		lprintf("Reply to (cursor) VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D is %#x\n",
			EndianSwap32Bit(((struct virtio_gpu_ctrl_hdr *)VTBuffers[0][1])->le32_type));
	}
...
static OSStatus DrawHardwareCursor(VDDrawHardwareCursorRec *rec) {
	...
	obuf->le32_resource_id = EndianSwap32Bit(cursorVisible ? 57 : 0);
...
// 	lprintf("Reply to cursor update/move is %#x\n",
// 		EndianSwap32Bit(ibuf->le32_type));
